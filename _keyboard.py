...
GLOBAL_DEFAULT_SENDKEYS_TIMEOUT = 0.1 #seconds
...
def SendString(output):
    for event in keyboard_stream(output):
        SendInput(event)
        time.sleep(GLOBAL_DEFAULT_SENDKEYS_TIMEOUT)
        
def sendAltKey(key):
    SendInput(Keyboard(VK_ALT), Keyboard(key))
    time.sleep(0.2)
    SendInput(Keyboard(VK_ALT, KEYEVENTF_KEYUP),
              Keyboard(key, KEYEVENTF_KEYUP))
    time.sleep(0.2)
    
def keyDown(vk_key, extended = False, delay = GLOBAL_DEFAULT_SENDKEYS_TIMEOUT):
    flag = 0
    if extended:
        flag = KEYEVENTF_EXTENDEDKEY
    SendInput(Keyboard(vk_key, flag))
    time.sleep(delay)
    
def keyUp(vk_key, extended = False, delay = GLOBAL_DEFAULT_SENDKEYS_TIMEOUT):
    flag = 0
    if extended:
        flag = KEYEVENTF_EXTENDEDKEY
    SendInput(Keyboard(vk_key, flag | KEYEVENTF_KEYUP))
    time.sleep(delay)
    
def shiftDown():
    keyDown(VK_SHIFT, True)
    
def shiftUp():
    keyUp(VK_SHIFT, True)
    
def ctrlDown():
    keyDown(VK_CONTROL, True)
    
def ctrlUp():
    keyUp(VK_CONTROL, True)
    
def sendCtrlKey(key, delay = GLOBAL_DEFAULT_SENDKEYS_TIMEOUT):
    ctrlDown()
    sendKey(key, extended = True, delay = delay)
    ctrlUp()
    
def sendKey(vk_key, times = 1, extended = False, delay = GLOBAL_DEFAULT_SENDKEYS_TIMEOUT):
    flag = 0
    if extended:
        flag = KEYEVENTF_EXTENDEDKEY
    for _ in range(times):
        SendInput(Keyboard(vk_key, flag), Keyboard(vk_key, KEYEVENTF_KEYUP | flag))
        sleep(delay)  
            
def sendEnd(times = 1, extended = False):
    sendKey(VK_END, times = times, extended = extended)
    
def sendHome(times = 1, extended = False):
    sendKey(VK_HOME, times, extended)
...
def sendShiftedKey(key, times = 1):
    shiftDown()
    sendKey(key, times = times, extended = True)
    shiftUp()     

def sendShiftEnd(times = 1):
    sendShiftedKey(VK_END, times = times)
    
def sendShiftLeft(times = 1):
    sendShiftedKey(VK_LEFT, times = times)
    
def sendShiftHome(times = 1):
    sendShiftedKey(VK_HOME, times = times)
    
def sendShiftRight(times = 1):
    sendShiftedKey(VK_RIGHT, times = times)
    
def sendShiftTab(times = 1):
    sendShiftedKey(VK_TAB, times = times)
